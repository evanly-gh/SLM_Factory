...
_INTERNAL_DECISION_FIELDS = frozenset({
    # Deterministic fields generated by this validator. They are accepted on
    # defense-in-depth revalidation and overwritten from trusted state.
...
def _validate_decision_json(
    ...
    *,
    ...
    if intervention == "data_rebuild":
        ...
        plan, identity, stats = ensure_untried_data_rebuild_plan(plan, state)
        validated["data_rebuild"] = plan
        validated["data_rebuild_plan_identity"] = identity
        validated["data_rebuild_trial_counts"] = stats
...
- primary_strategy: one of resample_existing, preserve_elite_resample,
  mine_new_real_source, source_diversification, difficulty_weighted_sampling,
  targeted_synth_positive
- support_strategies: zero to two other eligible strategies
...
- elite.provenance: best_non_pruned_dataset or current_dataset
- elite.dataset_version: non-negative integer
...
- targeted_synth_positive is eligible only for classification and NER
- primary/support strategies must be unique; support is bounded to two
- declare at most one sampling strategy; resample_existing is primary-only
- preserve/mining/synthesis strategies require positive material budgets whose sum
  does not exceed target_rows
...
def _llm_iterate(state: AgentState) -> dict:
    ...
    rebuild_trials = tried_data_rebuild_plans(state)
    rebuild_trial_lines = "\n".join(
        "    - "
        f"identity={record['identity']} "
        f"score={record.get('score')} "
        f"pruned={record.get('pruned', False)} "
        f"yield={record.get('yield_status') or 'unknown'}"
        for record in rebuild_trials
    )
    rebuild_trials_block = (
        f"Data-rebuild plans tried={len(rebuild_trials)} "
        f"pruned={sum(1 for record in rebuild_trials if record.get('pruned'))} "
        f"zero_yield={sum(1 for record in rebuild_trials if record.get('yield_status') == 'no_novelty')}\n"
        f"{rebuild_trial_lines or '    - (none)'}"
    )
...
def apply_iteration_policy(score: float) -> dict:
    ...
    if score < 0.80:
        return {
            "band": "<0.80",
            "intervention": "data_rebuild",
            "data_rebuild_strategy": "resample_existing",
        ...
        return {
            "band": ">=0.95",
            "intervention": "data_rebuild",
            "data_rebuild_strategy": "targeted_synth_positive",
            "description": (
...
def iterate_node(state: AgentState) -> AgentState:
    ...
    try:
        ...
        if intervention == "data_rebuild":
            ...
            _log(
                model_id,
                "  Data rebuild: "
                f"primary={plan['primary_strategy']} "
                f"support={plan['support_strategies']} "
                f"identity={llm_decision['data_rebuild_plan_identity']}",
            )
    ...
    if intervention == "data_rebuild":
        if llm_decision is not None:
            ...
            rebuild_identity = llm_decision["data_rebuild_plan_identity"]
        else:
            ...
            rebuild_plan, rebuild_identity, _ = (
                ensure_untried_data_rebuild_plan(rebuild_plan, state)
            )
        state["data_rebuild_plan"] = rebuild_plan
        state["data_rebuild_plan_identity"] = rebuild_identity
