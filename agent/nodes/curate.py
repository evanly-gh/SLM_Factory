...
def curate_node(state: AgentState) -> AgentState:
    ...
    model_id = selected.label  # log prefix includes quant, e.g. "Qwen/... [Q4_K_M]"
    ...
    if intervention == "data_rebuild" or state["current_dataset_path"] is None:
        ...
        N_TOTAL = DATASET_SIZE_BY_TYPE.get(task_type, 150)
        ...
        if len(gold) < n_gold_target:
            _log(model_id,
                 ...
                 f"Raise the acquisition cap to grow the pool.")

        # Hard negatives: only meaningful for tasks where a CONTRASTIVE wrong-label example
        # is well-defined (classification/NER/generation). For math_reasoning/code_generation
        # there is NO safe wrong-answer negative — SFT on wrong answers teaches the model to
        # produce errors (B62) — so `synthesize_hard_negatives` just returns the gold examples
        # unchanged, which previously got appended as DUPLICATES (inflating the dataset and
        # showing "162 synthesized" while the report counted them all as gold). So for math/code
        # we skip hard negatives entirely: the dataset is GOLD-ONLY, and the report is accurate.
        if task_type in ("math_reasoning", "code_generation"):
            hard = []
            n_hard_effective = 0
            _log(model_id, "  Hard negatives: N/A for math/code (no safe wrong-answer negative, B62) "
                           "— dataset is gold-only")
        else:
            _log(model_id, "  Hard negatives: SYNTHETIC — orchestrator LLM (contrastive 2-for-1); "
                           "not scraped/downloaded")
            if failures:
                _log(model_id, f"  Using {min(len(failures), n_hard_target)} failure examples as seeds")
            # Preserve the intended ~65:35 gold:hard ratio even when gold is scarce (don't
            # let synthetic negatives dominate a small gold set).
            hard_ratio = n_hard_target / max(n_gold_target, 1)
            ...
            # Rotate generation temperature per rebuild so successive rebuilds differ (variety).
            _rebuild_temps = [0.7, 0.9, 1.1]
            gen_temperature = _rebuild_temps[int(state.get("dataset_version", 0)) % len(_rebuild_temps)]
            _log(model_id, f"  Hard-negative generation temperature: {gen_temperature} (rotates per rebuild)")
