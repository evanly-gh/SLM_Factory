...
# its rows. "data_rebuild" in a trajectory is ambiguous — a single rebuild routinely runs several
# of these (the plan's strategy, then resample-fill, then synth-fill), and reshuffling an existing
# pool, mining new real rows, targeting a confusion pair and padding to target are four very
# different interventions with four different expected effects. Every rebuild now names the kinds
# that actually contributed rows, so a log line stating `data_rebuild` always resolves to what ran.
REBUILD_KIND_NAMES: dict[str, str] = {
    # The universal filler that assembles every curriculum from the train pool. NOT an
    # orchestrator-selectable strategy any more — see agent/data_rebuild.py.
    "resample": "resample-fill",
    "mine_new_real_source": "mine-new-real",
    "surgical_synthesize": "surgical-synth",
    "synthesize": "fill-synth",
    # Retained for historical datasets only — synth-fill was removed 2026-08-16.
    "synth_fill": "synth-fill",
}
...
# synth-fill and generation-family positives were reported as unattributed and the dataset looked
# like it contained almost no synthetic data at all when most of the synthesis budget went there.
# `synthetic_fill` is retained so historical datasets and resumed checkpoints still count correctly;
# nothing produces it any more (synth-fill removed 2026-08-16).
...
def _pinned_label_space(state: AgentState) -> set[str] | None:
    ...
    return label_space_from_eval_set(state.get("eval_set"), state["task_type"])
...
# *viability*, not a target — the per-tier target is still computed by agent/data_sizing.py.
...
def curate_node(state: AgentState) -> AgentState:
    ...
    if not isinstance(plan, dict):
        ...
        plan = normalize_data_rebuild_plan(
            plan,
            populated_buckets=_populated_difficulty_buckets(state, model_id),
            task_type=task_type,
            hypothesis=hypothesis,
            target_rows=int(
    ...
    # The plan strategy names only the PRIMARY kind. resample-fill and synth-fill run on top of
    # it every time, so the banner announces the kinds this rebuild may use and the closing
    # "DATA REBUILD kinds" line reports which ones actually produced rows.
    _PLAN_PRIMARY_KIND = {
        # The universal filler that assembles every curriculum from the train pool. NOT an
    # orchestrator-selectable strategy any more — see agent/data_rebuild.py.
    "resample": "resample-fill",
        "acquire": "mine-new-real",
        "synthesize": "surgical-synth + fill-synth",
    }
    _log(
        model_id,
        f"DATA REBUILD: kind={_PLAN_PRIMARY_KIND.get(strategy, strategy)} "
        f"strategy={strategy} ({_SUBSTRATEGY_EFFECT.get(strategy, 'unknown')}) "
        f"target_rows={plan['target_rows']} seed={seed} "
        f"[reshuffle-fill and synth-fill may also run; see 'DATA REBUILD kinds' below]",
    )
    ...
    if strategy == "acquire":
        ...
        # Curated benchmark runs (SLM_BENCHMARK_TASK) carry NO task_plan at all, so
        # `load_benchmark_dataset` had nothing to resolve a benchmark key from and Stage-0 was
        # skipped entirely — mining went straight to paid agentic discovery even for a task whose
        # own benchmark is sitting in the local HF cache. Naming it here is what lets the
        # `routerbench` / `bc5cdr` aliases actually fire (B259).
        _benchmark = os.environ.get("SLM_BENCHMARK_TASK") or ""
        if _benchmark:
            task_plan.setdefault("benchmark", _benchmark)
        # OVERWRITE (not setdefault) the plan's labels with the run's pinned vocabulary. The plan
        # is what crosses into the agentic-discovery subprocess, and its `labels` is what the LLM
        # column-mapper is shown as the permitted target set. An orchestrator-authored label list
        # must never widen the space the eval set established (B259).
        _pinned = _pinned_label_space(state)
        if _pinned:
            task_plan["labels"] = sorted(_pinned)
    ...
    # Second half of the no-shrink contract (B257). resize_curriculum_for_tier ratchets the
    # per-tier TARGET, but the orchestrator also writes target_rows into a rebuild plan directly,
    # so the plan can still name a number below what is already curated. `allocate` stops at
    # target_rows, which would silently drop the surplus. Rows leave only via QC or the eval
    # firewall, so the effective target is never below the dataset we already have.
    if previous_rows and len(previous_rows) > target_rows:
        _log(
            model_id,
            f"  Target {target_rows} is below the current dataset ({len(previous_rows)} rows); "
            f"holding at {len(previous_rows)} — rows are dropped only by quality control",
        )
    ...
    # SYNTH-FILL REMOVED 2026-08-16. It padded every curriculum up to `target_rows` with generated
    # rows, and the evidence says that bought nothing:
    #   * the size target is itself a heuristic (a function of the zero-shot baseline and parameter
    #     count), so "reach the target" was never a real requirement;
    #   * BC5CDR converged at 0.8098 — the project's best result — on a GOLD-ONLY curriculum that
    #     ran ~7,100 rows BELOW its 8,929-row target;
    #   * the SFT literature is consistent that a small clean set beats a large noisy one (LIMA,
    #     AlpaGasus: 9k filtered beats 52k unfiltered);
    #   * it was where most of the teacher budget went — one traced rebuild spent 749 generations
    #     to keep 225 rows, of which 64 survived quality control.
    # Targeted synthesis (the `synthesize` strategy, including surgical confusion-pair rows) is
    # unaffected: it is chosen by the orchestrator against specific failure evidence, and its rows
    # inherit a real anchor's label. What is gone is padding for the sake of a row count.
    if len(dataset) < target_rows:
        _log(
            model_id,
            f"  Curriculum is {len(dataset)} row(s) against a {target_rows}-row target "
            f"(short by {target_rows - len(dataset)}). Synth-fill was removed on 2026-08-16 — "
            "padding to a heuristic target with generated rows is not worth the label risk. "
            "Training on real data only.",
        )
    _enforce_curriculum_floor(dataset, model_id=model_id, target_rows=target_rows)
    ...
    # Under-target is EXPECTED now that synth-fill is gone: the curriculum is whatever real data
    # supplies after quality control, and the target is a heuristic rather than a requirement. It
    # must not be silent though — a run training on 3,461 rows against a 5,000-row target should say
    # so plainly rather than leaving the gap to be found in the artifacts later (B228). The
    # MIN_CURRICULUM_ROWS floor above is what turns "smaller than hoped" into "not viable".
    if len(dataset) < target_rows:
        _log(
            model_id,
            f"  ⚠ PROCEEDING BELOW DATA TARGET: {len(dataset)} row(s) vs target {target_rows} "
            f"(short by {target_rows - len(dataset)}, {len(dataset) / max(target_rows, 1):.0%} of "
            f"target). This is real-data-only by design — synth-fill was removed 2026-08-16. "
            f"Above the {MIN_CURRICULUM_ROWS}-row viability floor. See the [qc] lines above for "
    ...
    # Name every rebuild KIND that actually contributed rows, with counts. This is the line that
    # makes a `data_rebuild` entry in the trajectory self-describing: reading it tells you whether
    # the turn reshuffled, mined, targeted a confusion pair, padded to target, or several at once.
    rebuild_kinds = [
        (rebuild_kind_name(origin), origin_composition.get(origin, 0),
         origin_novelty.get(origin, 0))
        for origin in sorted(
            origin_composition, key=lambda o: -origin_composition.get(o, 0)
        )
        if origin_composition.get(origin, 0)
    ]
    _log(
        model_id,
        f"  DATA REBUILD kinds (plan strategy={strategy}): "
        + "; ".join(
            f"{kind}={rows} row(s) ({novel} novel)"
            for kind, rows, novel in rebuild_kinds
        ),
    )
    ...
    state["last_curation"] = {
        ...
        # EVERY teacher-generated row, whichever path produced it. `n_hard_generated` counts only
        # the plan's `synthesize` strategy, so a rebuild that generated 990 rows and kept 93 of
        # them reported 29 — the run graphics drew a synthetic band of ~1% and the rest of the
        # generated data was indistinguishable from unattributed filler.
        "n_synth_total": sum(provenance.get(tag, 0) for tag in SYNTHETIC_PROVENANCE_TAGS),
        "n_synth_by_provenance": {
            tag: provenance.get(tag, 0)
            for tag in SYNTHETIC_PROVENANCE_TAGS
            if provenance.get(tag, 0)
        },
